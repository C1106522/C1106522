...
!#########################################################################################
      ...
      integer :: n,np,ng,ipts,iz,maxpts,maxsec,nn,ntran
...
112   format(3F25.7)
      ...
      write(23,*) 'Variables:'
      write(23,113) 'n','type','cxsect','cysect','czsect','radsect','heighsect','basesect', &
	...
	t_radsect(n)=0 ; t_dr(n)=0
      ...
      write(23,114) n,t_geomtype(n),t_cxsect(n),t_cysect(n),t_czsect(n),t_radsect(n), &
	...
	write(23,115) 'edge','tran_sect(X)','tran_sect(Y)','tran_egde(X)','tran_edge(Y)'
	do i=1,9
	  write(23,116) i,tran_sect(i,1,n),tran_sect(i,2,n),tran_edge(i,1,n),tran_edge(i,2,n)
	end do
      end if
      end do
      close(23)
...
113   format(2a5,14a15)
114   format(2I5,14F15.8)
115   format(a6,4a20)
116   format(I6,4F20.8)
      end program
!#########################################################################################
!#########################################################################################
...
!	    nx(n)=nxn
!	    do m=1,4
!	      ec(axe,ic)=epy(m,ix,n)
!	      ec(axe+inc,ic)=ecz(m,ix,n)
!	      cr(ic)=ecr(m,ix,n)
!	    end do
!	  end if
	...
	itun(n)=ipsect
	WRITE(6,'(a,2I10)') 'ipsect,ipts',ipsect,ipts 
