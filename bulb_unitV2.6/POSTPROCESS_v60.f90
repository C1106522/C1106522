...
!#########################################################################################
	   logical :: bulb_case
	   integer :: t_nsect
	   double precision :: dist,xval,yval,zval,rval,t_jc,t_kc,t_h,t_b,yi,ye,zi,ze
	   integer,allocatable,dimension(:) :: t_geomtype
	   double precision,allocatable,dimension(:) :: t_cxsect,t_cysect,t_czsect,t_djc,t_dkc
	   double precision,allocatable,dimension(:) :: t_radsect,t_height,t_base,t_dr,t_dh,t_db
	   ...
	   if(bulb_case.eq..TRUE.) then
	     ...
	     open(file=23,file='tunnel_spec.txt')
	     do m=1,3
		read(23,*)
	     end do
	     read(23,*) t_nsect
	      
	     allocate(t_geomtype(t_nsect),dxx(t_nsect),dyy(t_nsect),dzz(t_nsect))
	     allocate(t_cxsect(t_nsect),t_cysect(t_nsect),t_czsect(t_nsect))
	     allocate(t_radsect(t_nsect),t_height(t_nsect),t_base(t_nsect))
	     allocate(t_djc(t_nsect),t_dkc(t_nsect),t_dr(t_nsect),t_dh(t_nsect),t_db(t_nsect))

	     do ns=1,t_nsect
	       read(23,*) m,t_geomtype(m),t_cxsect(m),t_cysect(m),t_czsect(m),t_radsect(m), &
     &  t_height(m),t_base(m),t_djc(m),t_dkc(m),t_dr(m),t_dh(m),t_db(m),dxx(m),dyy(m),dzz(m) 
             end do
		 ...
		 if(xval.ge.t_cxsect(ns) .and. xval.lt.t_cxsect(ns+1)) then
		   ...
		   IF(t_geomtype(ns).eq.1 .and. t_geomtype(ns).eq.t_geomtype(ns+1)) THEN
		     ...
		     if(rp.ge.rval) then
		       ...
		       vis(i,j,k)=0 ; vism(i,j,k)=0 ; ksgs(i,j,k)=0 
		       eps(i,j,k)=0 !;epsm(i,j,k)=0
		     end if
		   
		   ELSE IF(t_geomtype(ns).eq.2 .and. t_geomtype(ns).eq.t_geomtype(ns+1)) THEN
		     dist=xval-t_geomtype(ns)
		     t_jc=t_cysect(ns)+t_djc(ns)*dist
		     t_kc=t_czsect(ns)+t_dkc(ns)*dist
		  ...
		  ELSE IF(t_geomtype(ns).ne.t_geomtype(ns+1) .or. t_geomtype(ns).eq.3 .and. &
     &			  t_geomtype(ns).eq.t_geomtype(ns+1)) then
	      ...
	      deallocate(t_cxsect,t_cysect,t_czsect,t_radsect,t_height,t_base)
	      deallocate(t_djc,t_dkc,t_dr,t_dh,t_db,dxx,dyy,dzz,t_geomtype)
	     end if
